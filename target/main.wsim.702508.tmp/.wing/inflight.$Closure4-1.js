module.exports = function({ $Utils, $apiUrl, $http_Util }) {
  class $Closure4 {
    ...
    async handle() {
      const response = (await $http_Util.get(String.raw({ raw: ["", "/hello"] }, $apiUrl),({"headers": ({"Accept": "application/json","Authorization": ("Basic " + (await $Utils.base64encode("admin:admin")))})})));
